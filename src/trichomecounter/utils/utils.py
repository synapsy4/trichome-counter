...
from trichomecounter.model import models
from trichomecounter.model import loss
from trichomecounter.paths import (
    RAW_DATA_DIR,
    PROCESSED_DATA_DIR,
    MODELS_DIR,
    CONFIG_DIR,
...
def get_random_data_paths(seed: int = None
    ...
    data_paths = [PROCESSED_DATA_DIR / "train",
    ...
    image_path_raw = RAW_DATA_DIR / (file_id + ".tiff")
    coord_path_raw = RAW_DATA_DIR / (file_id + "_coords.mat")
...
def init_model(cfg: dict[str, Any],
    ...
    if cfg["model"]["model_name"] in model_names:
        ...
        # Case 2: Continue training chosen => load model
            ...
            return load_model(cfg["model"]["model_name"], cp), continue_training
    ...
    # Case 2: Init model from another model path (transfer learning)
        ...
        return load_model(cfg["model"]["pre_model_name"], cfg["model"]["pre_cp"]), continue_training
...
def init_optimizer(model_params: Iterator[torch.nn.Parameter],
    ...
    if continue_training:
        ...
        model_dir = MODELS_DIR / cfg["model"]["model_name"]
